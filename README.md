...
```toml
[dependencies]
r402 = { version = "0.21", features = ["evm", "http"] }
# SVM + MCP, same crate:
# r402 = { version = "0.21", features = ["evm", "svm", "http", "mcp"] }
# Every production chain + HTTP + MCP:
# r402 = { version = "0.21", features = ["full"] }
```
...
Permit2 (`upto`): 402 carries `eip2612GasSponsoring` without `with_extension`. `erc20ApprovalGasSponsoring` only if the facilitator `/supported` lists it. `auto_approve(true)` is spec Option A and **only runs if no sponsoring extension was attached**. `new(signer)` cannot `readContract`; the first Permit2 payment is HTTP 412.

`.provider(...)` needs `r402-evm` `client-provider`. Umbrella `r402` `evm` does not enable it:

```toml
r402-evm = { version = "0.21", features = ["client-provider"] }
```
