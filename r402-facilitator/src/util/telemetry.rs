...
impl Telemetry {
    ...
    #[allow(clippy::option_if_let_else)] // both branches are complex init blocks
    pub fn register(&self) -> TelemetryProviders {
        let telemetry_protocol = TelemetryProtocol::from_env();
        if let Some(telemetry_protocol) = telemetry_protocol {
...
impl TelemetryProviders {
    ...
    /// ```
    #[must_use]
    #[allow(clippy::unused_self)] // &self kept for future extensibility
    pub fn http_tracing(
        &self,
    ) -> TraceLayer<
