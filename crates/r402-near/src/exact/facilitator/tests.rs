//! Unit tests for NEAR exact verify/settle, one case per `invalidReason`.
...
use crate::chain::rpc::{
    NearAccessKeyPermissionKind, NearAccessKeyView, NearAccountView, NearReceiptStatus, NearRpc,
    NearRpcError, NearSettlementOutcome, NearStorageBalance,
};
use crate::{
...
impl NearRpc for MockRpc {
    ...
    async fn ft_balance_of(&self, _token: &str, _account_id: &str) -> Result<u128, NearRpcError> {
        self.balance
...
fn signed_delegate_b64(opts: DelegateOpts) -> String {
    ...
    if opts.transfer_kind {
        ...
        if opts.extra_action {
            ...
            actions.push(NonDelegateAction::try_from(Action::FunctionCall(Box::new(fc2))).unwrap());
        }
    }
...
async fn rejects_wrong_x402_version() {
    ...
    let response = verify_request_json(&rpc, &relayers(), DEFAULT_MAX_SPONSORED_GAS, &req).await;
...
async fn rejects_invalid_signature_without_payer() {
    let (rpc, b64, accepted, reqs) = verify_ok().await;
    let mut decoded = super::decode_signed_delegate(&b64).unwrap();
...
async fn borsh_roundtrip_golden() {
    let b64 = signed_delegate_b64(DelegateOpts::default());
    let decoded = super::decode_signed_delegate(&b64).unwrap();
...
async fn settle_rejects_duplicate() {
    ...
    let req = request(&b64, &reqs, &reqs);
    let cache = SettlementCache::new();
    assert_eq!(cache.reserve(&b64), r402_core::cache::Duplicate::No);
    let response = settle_request(
        &rpc,
        &relayers(),
        &cache,
