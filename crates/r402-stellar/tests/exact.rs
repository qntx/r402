...
//! In-process exact-scheme tests. No live Soroban RPC. No HTTP E2E.

use r402_protocol::scheme::SchemeId;
use r402_stellar::chain::StellarChainReference;
use r402_stellar::{
    STELLAR_NETWORKS, StellarExact, USDC, USDC_PUBNET_ADDRESS, USDC_TESTNET_ADDRESS,
};

#[test]
fn crate_name_matches_directory() {
    assert_eq!(env!("CARGO_PKG_NAME"), "r402-stellar");
}

#[test]
fn stellar_exact_scheme_id() {
    assert_eq!(StellarExact.namespace(), "stellar");
    assert_eq!(StellarExact.scheme(), "exact");
    assert_eq!(StellarExact.caip_family(), "stellar:*");
}

#[test]
fn networks_table_has_pubnet_and_testnet() {
    assert_eq!(STELLAR_NETWORKS.len(), 2);
    let names: Vec<_> = STELLAR_NETWORKS.iter().map(|n| n.name).collect();
    assert_eq!(names, ["stellar", "stellar-testnet"]);
}
...
fn chain_reference_converts_to_caip2() {
    let chain: r402_protocol::ChainId = StellarChainReference::TESTNET.into();
    assert_eq!(chain.to_string(), "stellar:testnet");
}

#[test]
fn extra_fixture_round_trips() {
    let json = include_str!("../../../tests/fixtures/stellar/extra_sponsored.json");
    let extra: r402_stellar::exact::StellarExtra = serde_json::from_str(json).unwrap();
...
#[cfg(feature = "client")]
#[test]
fn find_default_stellar_asset_covers_usdc() {
    use r402_protocol::ChainId;
    use r402_stellar::find_default_stellar_asset;

    let pubnet: ChainId = "stellar:pubnet".parse().expect("pubnet");
    let usdc = find_default_stellar_asset(USDC_PUBNET_ADDRESS, &pubnet).expect("USDC");
    assert_eq!(usdc.symbol, "USDC");
    assert_eq!(usdc.decimals, 7);
    assert!(find_default_stellar_asset("not-an-asset", &pubnet).is_none());
}
...
fn try_new_question_mark() -> Result<(), r402_protocol::FacilitatorError> {
    use r402_stellar::chain::{StellarChainProvider, StellarSigner};
    use r402_stellar::exact::StellarExactFacilitator;

    let signer =
        StellarSigner::from_secret("SCKB3ECHCPVM4HJPNCQWTQWJJ5XRL6UNKLTTCIH4B7TB22NKJ5GUFMIV")
            .expect("oracle test secret");
    let provider = StellarChainProvider::new(StellarChainReference::TESTNET, vec![signer], None)
        .expect("testnet rpc url");
    let _fac = StellarExactFacilitator::try_new(provider)?;
    Ok(())
}
...
async fn try_new_supported_is_exact_on_provider_chain() {
    ...
    let provider = StellarChainProvider::new(StellarChainReference::TESTNET, vec![signer], None)
        .expect("testnet rpc url");
    let fac = StellarExactFacilitator::try_new(provider).expect("try_new");
    ...
    assert_eq!(kind.network, "stellar:testnet");
...
mod verify_settle {
    ...
    use r402_stellar::chain::StellarFacilitatorError;
    use r402_stellar::chain::rpc::{StellarRpc, StellarRpcError};
    use r402_stellar::chain::xdr::{
    ...
    use r402_stellar::exact::facilitator::{settle_request, verify_request_json};
    use r402_stellar::{
        BASE_FEE_STROOPS, DEFAULT_MAX_TRANSACTION_FEE_STROOPS, NULL_ACCOUNT, USDC_TESTNET_ADDRESS,
    };
    ...
    use stellar_xdr::{
        ...
        ContractEventType, ContractEventV0, ContractId, DiagnosticEvent, ExtensionPoint, Hash,
        HostFunction, Limits, PublicKey, ScVal, SequenceNumber, String32, Thresholds,
        TransactionEnvelope, TransactionExt, Uint256, WriteXdr,
    };
    ...
    fn transfer_event(from: &str, to: &str, amount: i128, asset: &str) -> DiagnosticEvent {
        use r402_stellar::chain::xdr::{sc_val_address, sc_val_i128};
    ...
    fn valid_tx() -> String {
        let host = r402_stellar::chain::xdr::transfer_host_function(
            USDC_TESTNET_ADDRESS,
            PAYER,
            PAY_TO,
            AMOUNT,
    ...
    async fn reject_wrong_function_name() {
        ...
        {
            let tx = inner_transaction_mut(&mut envelope).unwrap();
            let op = r402_stellar::chain::xdr::invoke_host_function_op_mut(tx).unwrap();
    ...
    async fn reject_op_source_and_muxed_facilitator() {
        ...
        let payload = r402_stellar::chain::ed25519_account_payload(FACILITATOR).unwrap();
        let muxed = format!(
            "{}",
    ...
    async fn golden_ts_transaction_xdr_decodes() {
        let raw = include_str!("../../../tests/fixtures/stellar/ts_transaction.b64").trim();
