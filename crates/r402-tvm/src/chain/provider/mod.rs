...
use super::account::TvmChainReference;
...
use crate::chain::codec::highload_v3::{
...
pub mod tonapi;
pub mod toncenter;
pub use toncenter::{TvmRestClient, default_base_url};
    ...
    async fn select_query_id(&self, for_emulation: bool) -> Result<u32, TvmRpcError> {
        ...
        let query_state = if account.is_active {
            if let Some(init) = &account.state_init {
                if crate::chain::codec::cell::cell_hash_hex(&init.code) != HIGHLOAD_V3_CODE_HASH {
                    return Err(TvmRpcError::Parse(
...
pub(crate) fn take_free_query_id(
    start_seqno: u32,
    query_state: Option<&crate::chain::codec::highload_v3::HighloadQueryState>,
