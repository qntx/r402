...
impl SchemeNetworkServer for AptosExact {
    ...
    fn enrich_payment_required_response<'a>(
        ...
        let changed = accepts.iter_mut().fold(false, |acc, req| {
            acc | (req.network == *ctx.network && apply_aptos_fee_payer(req, ctx.supported))
        });
