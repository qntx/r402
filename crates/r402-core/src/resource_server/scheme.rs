//! Per-scheme resource-server adapter (official `SchemeNetworkServer`).
...
/// Context for scheme 402 enrichment (official `SchemePaymentRequiredContext`).
#[derive(Debug)]
#[non_exhaustive]
...
///
/// Enrich hooks are AFIT async (official `Promise` return). Defaults are no-ops.
