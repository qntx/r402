...
//! `specs/schemes/auth-capture/scheme_auth_capture_evm.md` and the official
//! TypeScript package.
...
/// Permit2 token permissions for auth-capture (local copy — no Eq on exact type).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
...
pub struct PaymentInfo {
    ...
    /// `requirements.asset`.
    pub token: Address,
    /// `requirements.amount` as uint120 (stored as U256 for convenience).
