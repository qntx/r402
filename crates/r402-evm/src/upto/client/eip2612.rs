...
//!
//! Buyers attach a signed [`Eip2612SignedPermit`] to the payment payload's
//! `extensions["eip2612GasSponsoring"]` slot. The facilitator then routes
//! the settlement through `proxy.settleWithPermit`, which atomically grants
//! the canonical Permit2 contract an allowance equal to the Permit2-signed
//! maximum before transferring funds. Resource servers benefit from
//! frictionless onboarding: buyers no longer need a pre-existing Permit2
//! approval transaction.
//!
//! Token nonces (`IERC20Permit::nonces(owner)`) and the EIP-712 domain
//! (`name`, `version`) are token-specific and MUST be supplied by the
//! caller. The
//! [`UptoPaymentRequirementsExtra`](crate::upto::UptoPaymentRequirementsExtra)
//! advertised by the resource server carries `name` and `version` fields
...
sol! {
    ...
    /// compliant ERC-20 implementation. Mirrored byte-for-byte against
    /// the canonical `OpenZeppelin` reference and EIP-2612 specification.
    #[derive(Debug)]
...
/// reverts with `Permit2612AmountMismatch` on divergence.
#[derive(Debug, Clone)]
pub struct Eip2612SigningParams {
    ...
    /// Token's EIP-712 domain `name` field (e.g. `"USD Coin"`). Read this
    /// from `paymentRequirements.extra.name`.
    pub token_name: String,
    /// Token's EIP-712 domain `version` field (e.g. `"2"`). Read this
    /// from `paymentRequirements.extra.version`.
    pub token_version: String,
    /// Permit owner address (== buyer / `payload.permit2Authorization.from`).
    pub owner: Address,
    /// Approved allowance — MUST equal the Permit2-signed maximum.
    pub value: U256,
    /// Owner's permit nonce on the token. Caller MUST query
    /// `IERC20Permit::nonces(owner)` immediately before signing.
    pub nonce: U256,
...
/// # Errors
///
/// Returns [`ClientError::Signing`] when the wallet refuses to sign or
/// the signature serialisation fails. Practically, the only realistic
/// failure mode is a hardware-wallet user rejection.
...
mod tests {
    ...
    async fn signing_is_deterministic_for_fixed_inputs() {
        // Two signs with the same wallet over the same typed data MUST
        // produce identical 65-byte signatures (RFC-6979 deterministic
        // ECDSA, used by alloy's local signer by default).
        let signer = PrivateKeySigner::random();
        let mut params = fixture();
        params.owner = signer.address();
    ...
    async fn signing_uses_canonical_permit2_spender() {
        // The spender MUST be the canonical Permit2 address regardless
        // of what the caller did — the proxy enforces this on-chain.
        let signer = PrivateKeySigner::random();
        let mut params = fixture();
        params.owner = signer.address();
