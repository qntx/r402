...
//!
//! A [`crate::Facilitator::settle`] implementation must write the broadcast
//! hash here on the first attempt and read it on retry so a pending
//! settlement is not broadcast twice.
...
/// Process-local [`PendingSettlementStore`] with lazy TTL pruning on [`Self::get`].
#[derive(Debug)]
