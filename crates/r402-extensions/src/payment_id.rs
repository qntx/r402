...
pub use r402_facilitator::Duplicate;
use r402_facilitator::TtlSet;
use r402_protocol::extension::{AdvertiseContext, Extension, SettleContext, VerifyContext};
...
mod tests {
    ...
    fn advertise_matches_official_shape() {
        ...
        let Some(entry) = ext.advertise(&AdvertiseContext::new(None)) else {
            panic!("payment-identifier advertise must emit an entry");
        };
        let value = entry.to_value();
        assert_eq!(value["info"]["required"], true);
        assert!(value.get("schema").is_some(), "schema must be advertised");
    ...
    fn validate_id_format_rules() {
        assert!(
            validate_id_format("pay_0123456789ab").is_ok(),
            "16-char alphanumeric id is valid"
        );
        assert!(
            validate_id_format("short").is_err(),
            "id shorter than 16 is invalid"
        );
        assert!(
            validate_id_format("has space!!!!!!").is_err(),
    ...
    fn validate_optional_missing_ok() {
        ...
        assert_eq!(
            ext.validate_payload_entry(None)
                .expect("optional missing id is ok"),
            None
        );
    }
}
