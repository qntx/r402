...
//!
//! Casper wire values (account hashes, contract package hashes, public keys,
//! signatures, nonces) are transported as unprefixed lowercase hex. Only the
//! handful of primitives below are needed, so the crate decodes them inline
//! instead of pulling a general-purpose hex dependency — this keeps the
//! footprint comparable to the other chain crates.
...
pub fn decode(input: &str) -> Result<Vec<u8>, HexDecodeError> {
    ...
    while index < bytes.len() {
        let hi = nibble(bytes.get(index).copied().unwrap_or_default())?;
        let lo = nibble(bytes.get(index + 1).copied().unwrap_or_default())?;
        out.push((hi << 4) | lo);
        index += 2;
    }
    Ok(out)
