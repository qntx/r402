//! Wire payload types for the Casper exact scheme.
...
use crate::chain::{Address, PublicKey, codec};
...
/// included in a block before it expires.
...
impl ExactCasperAuthorization {
    ...
    pub fn nonce_bytes(&self) -> Result<[u8; NONCE_LEN], CasperExactError> {
        codec::decode_exact::<NONCE_LEN>(&self.nonce)
    ...
    pub const fn validity_window(&self) -> Option<u64> {
        self.valid_before
            .as_secs()
            .checked_sub(self.valid_after.as_secs())
    }
}
...
impl ExactCasperPayload {
    ...
    pub fn signature_bytes(&self) -> Result<[u8; SIGNATURE_LEN], CasperExactError> {
        codec::decode_exact::<SIGNATURE_LEN>(&self.signature)
            .map_err(|e| CasperExactError::InvalidSignature(e.to_string()))
    }
}

/// Validates structural invariants of a payload: fixed-width signature,
...
pub mod v2 {
    ...
    use super::{
        CasperPaymentRequirementsExtra, ExactCasperPayload, ExactScheme, SupportedPaymentKindExtra,
    };
    ...
    pub type PaymentPayload = WirePayload<PaymentRequirements, ExactCasperPayload>;

    /// Type alias for payment requirements with Casper-specific types.
    ///
    /// `asset` is a contract package hash while `payTo` is a tagged
    /// addressable key; both stay wire-level strings here.
    pub type PaymentRequirements =
        WireReqs<ExactScheme, Motes, CompactString, CasperPaymentRequirementsExtra>;
...
mod tests {
    ...
    fn payload_round_trips_reference_sdk_json() {
        ...
        assert_eq!(payload.authorization.valid_after.as_secs(), 1_700_000_000);
        assert_eq!(payload.authorization.valid_before.as_secs(), 1_700_000_600);
    ...
    fn typed_verify_request_decodes_v2_envelope() {
        ...
        let typed =
            v2::VerifyRequest::from_verify(r402_protocol::payment::VerifyRequest::from(request))
                .unwrap();
    ...
    #[test]
    fn shape_rejects_public_key_mismatch() {
        let mut json = payload_json();
        json["publicKey"] = serde_json::json!(format!("01{}", "bb".repeat(32)));
        json["signature"] = serde_json::json!(format!("01{}", "aa".repeat(64)));
        let payload: ExactCasperPayload = serde_json::from_value(json).unwrap();
        assert_eq!(
            validate_payload_shape(&payload).unwrap_err(),
            CasperExactError::PublicKeyMismatch
        );
    }
}
