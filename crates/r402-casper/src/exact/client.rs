...
use r402_protocol::{
    Base64Bytes, ChainId, ClientError, PaymentRequired, ResourceInfo, UnixTimestamp,
};
use rand::RngExt;

use crate::chain::motes::Motes;
use crate::chain::{Address, ContractPackageHash, PublicKey, codec};
use crate::exact::eip712::{Eip712Domain, domain_from_parts, transfer_with_authorization_digest};
use crate::exact::payload::{
    self, ExactCasperAuthorization, ExactCasperPayload, NONCE_LEN, validate_payload_shape,
};
use crate::exact::{CasperExact, CasperExactError};

/// Field names and Solidity-style types of
/// [`PRIMARY_TYPE`](crate::exact::eip712::PRIMARY_TYPE), in signing order.
...
///
/// The returned signature MUST be the full 65-byte wire form: algorithm tag
/// (`0x01` / `0x02`) followed by 64 signature bytes.
...
pub fn build_payment_payload(
    ...
    r402_protocol::payment::PaymentPayload::new(accepted, payload)
}
...
/// Casper exact-scheme client that plugs into `r402-client`.
#[derive(Clone)]
    ...
    S: CasperSigner + Sync,
{
    fn sign_payment(
        ...
        Box::pin(async move {
            ...
            let signature_hex = codec::encode(&signature);
...
mod tests {
    ...
    fn requirements() -> payload::v2::PaymentRequirements {
    ...
    impl CasperSigner for StubSigner {
        ...
        fn sign_digest(
            ...
            let mut sig = [0u8; 65];
            sig[0] = 0x02;
            for byte in sig.iter_mut().skip(1) {
                *byte = 0xaa;
            }
    ...
    fn builder_derives_window_from_requirements() {
        let auth =
            AuthorizationBuilder::for_requirements(PAYER.parse().unwrap(), &requirements(), 1_000)
                ...
                .nonce([7u8; NONCE_LEN])
                .build();
        assert_eq!(auth.valid_after.as_secs(), 400);
        assert_eq!(auth.valid_before.as_secs(), 1_300);
    ...
    fn builder_saturates_instead_of_overflowing() {
        ...
        .build();
        assert_eq!(auth.valid_before.as_secs(), u64::MAX);
    }
