...
//!
//! CSPR has 9 decimals (1 CSPR = `1_000_000_000` motes). wCSPR inherits that
//! scale. Sub-mote fractional digits are rejected — truncating would make the
//! signed amount disagree with the settled amount.
...
impl FromStr for Motes {
    ...
    /// Parses a base-unit (mote) integer string. Use [`Motes::from_cspr_str`]
    /// for human decimal CSPR amounts such as `"1.5"`.
