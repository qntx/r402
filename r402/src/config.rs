...
impl<T> LiteralOrEnv<T> {
    /// Creates a new `LiteralOrEnv` from a literal value.
    pub const fn from_literal(value: T) -> Self {
        Self(value)
    }
...
/// Default configuration values and environment variable resolution.
pub mod config_defaults {
    ...
    /// # Panics
    ///
    /// Panics if `DEFAULT_HOST` is not a valid `IPv4` address (unreachable in practice).
    #[must_use]
    pub fn default_host() -> IpAddr {
        env::var("HOST")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| IpAddr::V4(DEFAULT_HOST.parse().expect("valid default host")))
    }
}
...
impl<TChainsConfig> Config<TChainsConfig> {
    ...
    pub const fn schemes(&self) -> &Vec<SchemeConfig> {
        &self.schemes
    }
    ...
    pub const fn chains(&self) -> &TChainsConfig {
        &self.chains
    }
}
